"use client";

import { Zap, Brain } from "lucide-react";

const allTechnologies = [
    { name: "React", category: "Frontend", color: "#61DAFB" },
    { name: "Next.js", category: "Frontend", color: "#6366f1" },
    { name: "JavaScript", category: "Frontend", color: "#F7DF1E" },
    { name: "TypeScript", category: "Frontend", color: "#3178C6" },
    { name: "Tailwind CSS", category: "Frontend", color: "#06B6D4" },
    { name: "Framer Motion", category: "Frontend", color: "#FF0055" },
    { name: "Redux Toolkit", category: "Frontend", color: "#764ABC" },

    { name: "Node.js", category: "Backend", color: "#339933" },
    { name: "Express", category: "Backend", color: "#94a3b8" },
    { name: "GraphQL", category: "Backend", color: "#E10098" },
    { name: "REST APIs", category: "Backend", color: "#00D9FF" },
    { name: "WebSockets", category: "Backend", color: "#00E5FF" },

    { name: "React Native", category: "Mobile", color: "#61DAFB" },
    { name: "Kotlin", category: "Mobile", color: "#7F52FF" },
    { name: "Expo", category: "Mobile", color: "#a78bfa" },

    { name: "AWS", category: "Cloud", color: "#FF9900" },
    { name: "Vercel", category: "Cloud", color: "#94a3b8" },
    { name: "Render", category: "Cloud", color: "#94a3b8" },

    { name: "VPS", category: "VPS", color: "#94a3b8" },

    { name: "MongoDB", category: "Database", color: "#47A248" },
    { name: "Redis", category: "Database", color: "#DC382D" },
    { name: "Firebase", category: "Database", color: "#FFCA28" },
    { name: "MySQL", category: "Database", color: "#00758F" },
];

const categories = [
    "Frontend",
    "Backend",
    "Mobile",
    "Cloud",
    "VPS",
    "Database",
];

export default function TechStack() {
    return (
        <section className="bg-slate-900 py-20 sm:py-28 px-4 sm:px-6">
            <div className="max-w-6xl mx-auto">

                {/* ── Header ── */}
                <div className="mb-12 sm:mb-16">
                    <span className="inline-flex items-center gap-2 rounded-full border border-slate-700 bg-slate-800 px-3 py-1 text-xs font-medium tracking-wide text-slate-300 uppercase mb-4">
                        <Brain className="w-3.5 h-3.5 text-sky-400" />
                        Our Technologies
                    </span>
                    <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3">
                        Technology <span className="text-sky-400">Stack</span>
                    </h2>
                    <p className="text-sm sm:text-base text-slate-400 max-w-xl leading-relaxed">
                        A modern, battle-tested set of tools across frontend, backend,
                        mobile, cloud, and databases.
                    </p>
                </div>

                {/* ── Stats row ── */}
                <div className="flex flex-wrap gap-6 mb-10 sm:mb-12">
                    <div className="flex items-center gap-2">
                        <Zap className="w-4 h-4 text-sky-400" />
                        <span className="text-xl font-bold text-white">
                            {allTechnologies.length}
                        </span>
                        <span className="text-sm text-slate-400">technologies</span>
                    </div>
                    <div className="w-px bg-slate-700 self-stretch hidden sm:block" />
                    <div className="flex items-center gap-2">
                        <Brain className="w-4 h-4 text-sky-400" />
                        <span className="text-xl font-bold text-white">
                            {categories.length}
                        </span>
                        <span className="text-sm text-slate-400">categories</span>
                    </div>
                </div>

                {/* ── Categories ── */}
                <div className="space-y-10">
                    {categories.map((category) => {
                        const techs = allTechnologies.filter(
                            (t) => t.category === category
                        );
                        return (
                            <div key={category}>
                                {/* Category label */}
                                <div className="flex items-center gap-3 mb-4">
                                    <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-widest">
                                        {category}
                                    </h3>
                                    <div className="flex-1 h-px bg-slate-800" />
                                </div>

                                {/* Tech chips */}
                                <div className="flex flex-wrap gap-2">
                                    {techs.map((tech) => (
                                        <div
                                            key={tech.name}
                                            className="group inline-flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs sm:text-sm font-medium text-slate-100 hover:border-slate-500 hover:bg-slate-700 transition-colors duration-150 cursor-default"
                                        >
                                            <span
                                                className="h-2 w-2 rounded-full flex-shrink-0"
                                                style={{ backgroundColor: tech.color }}
                                            />
                                            {tech.name}
                                        </div>
                                    ))}
                                </div>
                            </div>
                        );
                    })}
                </div>

            </div>
        </section>
    );
}